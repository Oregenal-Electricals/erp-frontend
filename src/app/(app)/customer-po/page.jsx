...
export default function CustomerPoPage() {
  ...
  async function quickCreateCustomer() {
    ...
    if (res.ok) {
      ...
      setCustomerList((list) => [...list, data]);
      setCustomerSuggestOpen(false);
    } else {
      setError(
  ...
  async function handleSave() {
    ...
    if (form.poType === 'WRITTEN') {
      ...
      body.verbalConfirmedDate = form.verbalConfirmedDate
        ? new Date(form.verbalConfirmedDate).toISOString()
        : new Date().toISOString();
    }
    const url = editingId
      ? `${API}/customer-po/${editingId}`
      : `${API}/customer-po`;
  ...
  async function handleIncreaseSave() {
    ...
    if (increaseForm.poType === 'WRITTEN')
      body.customerPoNumber = increaseForm.customerPoNumber;
    else {
      body.verbalConfirmedBy = increaseForm.verbalConfirmedBy;
      body.verbalConfirmedDate = increaseForm.verbalConfirmedDate
        ? new Date(increaseForm.verbalConfirmedDate).toISOString()
        : new Date().toISOString();
    }
    const res = await fetch(
      `${API}/customer-po/${increaseModal.id}/increase-quantity`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  function openEdit(cpo) {
    setForm({
      ...
      verbalConfirmedDate: cpo.verbalConfirmedDate
  ...
  async function handleAction(id, action, body = {}) {
    ...
    if (res.ok) {
      fetchAll();
      if (viewDetail?.id === id) {
        const d = await fetch(`${API}/customer-po/${id}`, {
          headers: { Authorization: `Bearer ${getToken()}` },
        });
  ...
  async function openDetail(id) {
    ...
    const sRes = await fetch(`${API}/customer-po/${id}/shortages`, {
      headers: { Authorization: `Bearer ${getToken()}` },
    });
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">
              Log written or verbal customer purchase orders and check material
              availability
            </p>
          </div>
          <button
        ...
        {stats && (
          <div className="grid grid-cols-3 md:grid-cols-8 gap-3 mb-6">
            {[
              ...
              {
                label: 'Order Value',
                value: fmt(stats.totalOrderValue),
                color: 'bg-teal-50',
              },
            ].map((s) => (
              <div
                ...
                <div key={c.id} className="p-4 hover:bg-gray-50">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      ...
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium ${PO_TYPE_COLORS[c.poType]}`}
                      >
                        {c.poType}
                      </span>
                      <span
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        onClick={() => openDetail(c.id)}
                        className="px-3 py-1.5 text-xs font-medium border border-gray-300 text-gray-700 bg-white rounded hover:bg-gray-100"
                      >
                        View
                      </button>
                    </div>
            ...
            <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-screen overflow-y-auto">
              <div className="p-6 border-b flex justify-between sticky top-0 bg-white">
                <div>
                  ...
                  <div className="flex gap-2 mt-1">
                    <span
                      className={`px-2 py-1 rounded text-xs font-medium ${PO_TYPE_COLORS[viewDetail.poType]}`}
                    >
                      {viewDetail.poType}
                    </span>
                    <span
                      className={`px-2 py-1 rounded text-xs font-medium ${STATUS_COLORS[viewDetail.status]}`}
                    >
                      {viewDetail.status}
                    </span>
                  </div>
              ...
              <div className="p-6">
                <div className="grid grid-cols-2 gap-6 mb-6">
                  <div>
                    ...
                    <div className="font-semibold">
                      {viewDetail.customerName}
                    </div>
                    {viewDetail.customerEmail && (
                      <div className="text-sm text-gray-500">
                        {viewDetail.customerEmail}
                      </div>
                    )}
                    {viewDetail.customerPhone && (
                      <div className="text-sm text-gray-500">
                        {viewDetail.customerPhone}
                      </div>
                    )}
                      ...
                      <>
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {viewDetail.verbalConfirmedBy}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">Confirmed Date:</span>
                          <span>{fmtDate(viewDetail.verbalConfirmedDate)}</span>
                        </div>
                      </>
                    )}
                    <div className="flex justify-between">
                      <span className="text-gray-500">PO Date:</span>
                      <span>{fmtDate(viewDetail.poDate)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Delivery Date:</span>
                      <span className="font-medium">
                        {fmtDate(viewDetail.deliveryDate)}
                      </span>
                    </div>
                    {viewDetail.mrpRunAt && (
                      <div className="flex justify-between">
                        ...
                        <span>{fmtDateTime(viewDetail.mrpRunAt)}</span>
                      </div>
                    )}
                    {viewDetail.acknowledgedDate && (
                      <div className="flex justify-between">
                        <span className="text-gray-500">Acknowledged:</span>
                        <span className="text-blue-600">
                          {fmtDate(viewDetail.acknowledgedDate)}
                        </span>
                      </div>
                    )}
                    {viewDetail.cancelReason && (
                      <div className="mt-2 p-2 bg-red-50 rounded text-xs text-red-600">
                        Cancelled: {viewDetail.cancelReason}
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex justify-end mb-6">
                  <div className="w-64 space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span>{fmt(viewDetail.subtotal)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Total GST:</span>
                      <span>{fmt(viewDetail.totalGst)}</span>
                    </div>
                    <div className="flex justify-between font-bold text-lg border-t pt-2">
                      <span>Total:</span>
                      <span className="text-blue-700">
                        {fmt(viewDetail.totalAmount)}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="border rounded-lg p-4 bg-gray-50">
                  <div className="flex justify-between items-center mb-3">
                    <h3 className="font-semibold text-gray-700">
                      ...
                      <span className="text-xs font-normal text-gray-400">
                        (runs automatically when created; re-check if stock or
                        other orders changed)
                      </span>
                    </h3>
                    <button
                      onClick={() => runRecheck(viewDetail.id)}
                      disabled={recheckSaving}
                      className="px-3 py-1.5 text-xs bg-gray-600 text-white rounded hover:bg-gray-700 disabled:opacity-50"
                    >
                      {recheckSaving ? 'Checking...' : '↻ Re-check'}
                    </button>
                  </div>
                  ...
                  {shortages?.itemResults ? (
                    <div className="space-y-2">
                      {shortages.itemResults.map((item, i) => {
                        ...
                        return (
                          <div
                            key={i}
                            className={`rounded p-3 text-sm border ${style}`}
                          >
                      ...
                      <div className="text-xs text-gray-500 mt-2">
                        {shortages.openCount > 0
                          ...
                          : shortages.itemResults.every(
                                (i) =>
                                  i.status === 'CHECKED' ||
                                  i.status === 'CHECKED_DIRECT_STOCK',
                              )
                    ...
                    <div className="text-xs text-gray-400">
                      Shortage check has not run yet for this PO.
                    </div>
                  )}
                </div>
                ...
                {viewDetail.amendmentOf && (
                  <div className="mt-4 p-3 bg-purple-50 rounded text-xs text-purple-700">
                    This is an increase-quantity order against{' '}
                    <span className="font-mono font-semibold">
                      {viewDetail.amendmentOf.cpoNumber}
                    </span>{' '}
                    ({viewDetail.amendmentOf.customerPoNumber})
                  </div>
                )}

                {viewDetail.amendmentChildren &&
                  viewDetail.amendmentChildren.length > 0 && (
                    <div className="mt-4">
                      <div className="text-xs font-semibold text-gray-600 mb-2">
                        Linked Increase Orders
                      </div>
                      <div className="space-y-1">
                        {viewDetail.amendmentChildren.map((child) => (
                          <div
                            key={child.id}
                            className="flex justify-between items-center bg-purple-50 rounded p-2 text-xs cursor-pointer hover:bg-purple-100"
                          ...
                          >
                            ...
                            <span
                              className={`px-2 py-0.5 rounded-full font-medium ${STATUS_COLORS[child.status]}`}
                            >
                              {child.status}
                            </span>
                            <span className="font-semibold">
                              {fmt(child.totalAmount)}
                            </span>
                ...
                <DocumentAttachments
                  ...
                  referenceNumber={viewDetail?.cpoNumber}
              ...
              <div className="p-6 border-t flex justify-end gap-3 sticky bottom-0 bg-white">
                {viewDetail.status === 'RECEIVED' && (
                  <button
                    onClick={() => openEdit(viewDetail)}
                    className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg text-sm hover:bg-gray-50"
                  >
                    Edit
                  </button>
                )}
                {!['RECEIVED', 'CANCELLED'].includes(viewDetail.status) && (
                  <button
                    onClick={() => openIncrease(viewDetail)}
                    className="px-4 py-2 border border-purple-300 text-purple-700 rounded-lg text-sm hover:bg-purple-50"
                  >
                    + Increase Quantity
                  </button>
                )}
                {viewDetail.status === 'RECEIVED' && (
                  <button
                    onClick={() => handleAction(viewDetail.id, 'acknowledge')}
                    className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm"
                  >
                    Acknowledge
                  </button>
                )}
                {!['COMPLETED', 'CANCELLED'].includes(viewDetail.status) && (
                  <button
                    onClick={() => {
                      setCancelModal(viewDetail.id);
                      setCancelReason('');
                    }}
            ...
            <div className="bg-white rounded-xl shadow-xl w-full max-w-5xl max-h-screen overflow-y-auto">
              <div className="p-6 border-b flex justify-between sticky top-0 bg-white">
                <h2 className="text-lg font-bold text-blue-700">
                  {editingId ? 'Edit Customer PO' : 'New Customer PO'}
                </h2>
                <button
                  onClick={() => {
                    setShowModal(false);
                ...
                <div>
                  ...
                  <div className="flex gap-3">
                    <button
                      type="button"
                      onClick={() =>
                        setForm((f) => ({ ...f, poType: 'WRITTEN' }))
                      }
                      className={`flex-1 px-4 py-3 rounded-lg border-2 text-sm font-medium ${form.poType === 'WRITTEN' ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-gray-200 text-gray-500'}`}
                    >
                      📄 Written PO
                      <div className="text-xs font-normal mt-1">
                        Customer sent a real PO document/number
                      </div>
                    </button>
                    <button
                      type="button"
                      onClick={() =>
                        setForm((f) => ({ ...f, poType: 'VERBAL' }))
                      }
                      className={`flex-1 px-4 py-3 rounded-lg border-2 text-sm font-medium ${form.poType === 'VERBAL' ? 'border-purple-500 bg-purple-50 text-purple-700' : 'border-gray-200 text-gray-500'}`}
                    >
                      📞 Verbal PO
                      <div className="text-xs font-normal mt-1">
                        Confirmed by phone, no document yet
                      </div>
                    </button>
                  </div>
                </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setForm((f) => ({
                              ...f,
                              verbalConfirmedBy: e.target.value,
                            }))
                          }
                          placeholder="e.g. Rahul - phone call"
                        />
                      </div>
                      <div>
                        <label className="block text-sm text-gray-600 mb-1">
                        ...
                        <input
                          type="date"
                          className="w-full border rounded-lg px-3 py-2 text-sm"
                          value={form.verbalConfirmedDate}
                          onChange={(e) =>
                            setForm((f) => ({
                              ...f,
                              verbalConfirmedDate: e.target.value,
                            }))
                          }
                        />
                      </div>
                  ...
                  <div className="col-span-2">
                    ...
                    {customerAddresses.length > 1 && (
                      <select
                        ...
                        onChange={(e) => {
                          const a = customerAddresses.find(
                      ...
                      >
                        <option value="">
                          — Pick a different saved address (
                          {customerAddresses.length} on file) —
                        </option>
                        {customerAddresses.map((a) => (
                          <option key={a.id} value={a.id}>
                            {a.addressType}: {a.addressLine}, {a.city}
                          </option>
                        ))}
                      </select>
                    )}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
              ...
              <div className="p-6 border-t flex justify-end gap-3">
                ...
                <button
                  onClick={async () => {
                    await handleAction(cancelModal, 'cancel', { cancelReason });
                    setCancelModal(null);
                  }}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-screen overflow-y-auto">
              <div className="p-6 border-b flex justify-between sticky top-0 bg-white">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Creates a new PO for the extra quantity, linked to{' '}
                    <span className="font-mono">{increaseModal.cpoNumber}</span>
                    . The original stays unchanged.
                  </p>
                </div>
                <button
                  onClick={() => setIncreaseModal(null)}
                  className="text-gray-400 text-xl"
                >
                  ✕
                </button>
              ...
              <div className="p-6 space-y-6">
                ...
                <div>
                  <label className="block text-sm text-gray-600 mb-2">
                    How was this increase communicated? *
                  </label>
                  <div className="flex gap-3">
                    <button
                      type="button"
                      onClick={() =>
                        setIncreaseForm((f) => ({ ...f, poType: 'WRITTEN' }))
                      }
                      className={`flex-1 px-4 py-2 rounded-lg border-2 text-sm font-medium ${increaseForm.poType === 'WRITTEN' ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-gray-200 text-gray-500'}`}
                    >
                      📄 Written
                    </button>
                    <button
                      type="button"
                      onClick={() =>
                        setIncreaseForm((f) => ({ ...f, poType: 'VERBAL' }))
                      }
                      className={`flex-1 px-4 py-2 rounded-lg border-2 text-sm font-medium ${increaseForm.poType === 'VERBAL' ? 'border-purple-500 bg-purple-50 text-purple-700' : 'border-gray-200 text-gray-500'}`}
                    >
                      📞 Verbal
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {increaseForm.poType === 'WRITTEN' ? (
                    <div className="col-span-2">
                      ...
                      <input
                        className="w-full border rounded-lg px-3 py-2 text-sm font-mono"
                        value={increaseForm.customerPoNumber}
                        onChange={(e) =>
                          setIncreaseForm((f) => ({
                            ...f,
                            customerPoNumber: e.target.value,
                        ...
                        placeholder="e.g. PO-2026-4521-A"
                      />
                    </div>
                  ) : (
                    <div className="col-span-2 grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          className="w-full border rounded-lg px-3 py-2 text-sm"
                          value={increaseForm.verbalConfirmedBy}
                          onChange={(e) =>
                            setIncreaseForm((f) => ({
                              ...f,
                              verbalConfirmedBy: e.target.value,
                      ...
                      <div>
                        ...
                        <input
                          type="date"
                          className="w-full border rounded-lg px-3 py-2 text-sm"
                          value={increaseForm.verbalConfirmedDate}
                          onChange={(e) =>
                            setIncreaseForm((f) => ({
                              ...f,
                              verbalConfirmedDate: e.target.value,
                  ...
                  <div className="col-span-2">
                    <label className="block text-sm text-gray-600 mb-1">
                      Delivery Date for Extra Quantity *
                    </label>
                    <input
                      type="date"
                      className="w-full border rounded-lg px-3 py-2 text-sm"
                      value={increaseForm.deliveryDate}
                      onChange={(e) =>
                        setIncreaseForm((f) => ({
                          ...f,
                ...
                <div>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 text-xs text-gray-500">
                      <tr>
                        {['Item', 'Additional Qty', 'Unit Price', 'Total'].map(
                          (h) => (
                            <th key={h} className="px-2 py-2 text-left">
                              {h}
                            </th>
                          ...
                          <tr key={i} className="border-b">
                            ...
                            <td className="px-2 py-1">
                              <input
                                ...
                                value={item.qty}
                                onChange={(e) =>
                                  updateIncreaseItem(i, 'qty', e.target.value)
                                }
                                placeholder="0"
                              />
                            </td>
                            ...
                            <td className="px-2 py-1 text-xs font-bold text-purple-700">
                              {fmt(c.total)}
                            </td>
                          </tr>
                        );
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Any additional context for this increase..."
                  />
                </div>
              </div>
              <div className="p-6 border-t flex justify-end gap-3 sticky bottom-0 bg-white">
                ...
                <button
                  onClick={handleIncreaseSave}
                  disabled={increaseSaving}
                  className="px-4 py-2 bg-purple-600 text-white rounded-lg text-sm disabled:opacity-50"
                >
                  {increaseSaving ? 'Creating...' : 'Create Increase Order'}
                </button>
              </div>
            </div>
          </div>
      ...
      {customerSuggestOpen &&
        ...
        createPortal(
          <div
            ...
            style={{
              top: customerSuggestPos.top,
              left: customerSuggestPos.left,
            }}
          >
            {!matchingCustomers(form.customerName).some(
              ...
              >
                {matches.map((p) => (
                  ...
                  >
                    ...
                    {p._source && (
                      <span
                        className={`ml-2 text-[10px] px-1.5 py-0.5 rounded ${p._source === 'Product' ? 'bg-indigo-50 text-indigo-600' : 'bg-amber-50 text-amber-700'}`}
                      >
                        {p._source}
                      </span>
                    )}
