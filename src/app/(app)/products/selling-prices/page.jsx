...
export default function ProductSellingPricesPage() {
  ...
  async function loadHistory(productId) {
    ...
    const res = await fetch(`${API}/products/selling-prices/product/${productId}`, {
      headers: { Authorization: `Bearer ${getToken()}` },
    });
  ...
  async function submit() {
    ...
    const url = isFirstTime
      ? `${API}/products/selling-prices`
      : `${API}/products/selling-prices/product/${selectedProduct.id}/revise`;
    const body = isFirstTime
      ? { productId: selectedProduct.id, sellingPrice: Number(price), ...(effectiveFrom ? { effectiveFrom } : {}) }
      : { sellingPrice: Number(price), effectiveFrom };
          ...
          <div className="space-y-4">
            ...
            {!loadingHistory && current && (
              <div className="bg-green-50 rounded-xl p-4">
                <div className="text-xs text-gray-500 mb-1">Current Selling Price</div>
                <div className="text-2xl font-bold text-green-700">₹{current.sellingPrice}</div>
            ...
            {!loadingHistory && (
              <div className="bg-white rounded-xl border p-4">
                <div className="font-semibold text-gray-700 mb-3">
                  {isFirstTime ? 'Set Price (first time for this product)' : 'Revise Price'}
                </div>
                {!isFirstTime && (
                  <p className="text-xs text-gray-400 mb-3">Product is already selected above - no need to search again. The current price stays valid for its period; your new value applies only from the date below.</p>
                )}
                <div className="flex gap-3 items-end flex-wrap">
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">Selling Price (₹)</label>
                    <input type="number" step="0.01" className="border rounded-lg px-3 py-2 text-sm w-40" value={price} onChange={e => setPrice(e.target.value)} />
                  </div>
                  <div>
                  ...
                  <button
                    onClick={submit}
                    disabled={saving || !price || (!isFirstTime && !effectiveFrom)}
                    className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 disabled:opacity-50"
                  >
                    {saving ? 'Saving...' : isFirstTime ? 'Set Price' : 'Revise Price'}
                  </button>
                </div>
              ...
              <div className="bg-white rounded-xl border shadow-sm">
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 text-gray-500 text-xs uppercase">
                    <tr>{['Price (₹)', 'Effective From', 'Effective To', 'Status'].map(h => <th key={h} className="px-3 py-2 text-left">{h}</th>)}</tr>
                  </thead>
                  <tbody className="divide-y">
                    {history.map((v, i) => (
                      <tr key={i} className={!v.effectiveTo ? 'bg-green-50' : ''}>
                        <td className="px-3 py-2 text-xs font-bold">{v.sellingPrice}</td>
                        ...
                        <td className="px-3 py-2 text-xs">{!v.effectiveTo ? <span className="px-2 py-1 rounded-full bg-green-100 text-green-700 text-xs">Active</span> : <span className="text-gray-400">Historical</span>}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
