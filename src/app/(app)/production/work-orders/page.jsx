...
export default function WorkOrdersPage() {
  ...
  async function handleExpand(id) {
    ...
    setExpandedId(id);
    const res = await fetch(`${API}/work-orders/${id}`, { headers: { Authorization: `Bearer ${getToken()}` } });
    if (res.ok) {
      const data = await res.json();
              ...
              <div key={wo.id} className="p-4">
                <div className="flex items-center justify-between cursor-pointer" onClick={() => handleExpand(wo.id)}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {wo.status === 'RELEASED' && <button onClick={e=>{e.stopPropagation();handleAction(wo.id,'start')}} className="px-2 py-1 text-xs bg-yellow-500 text-white rounded">Start</button>}
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input className="w-full border rounded-lg px-3 py-2 text-sm font-mono" value={form.productCode} onChange={e=>setForm(f=>({...f,productCode:e.target.value}))} />
                  </div>
                  <div>
                    <label className="block text-sm text-gray-600 mb-1">Product Name *</label>
                    <input className="w-full border rounded-lg px-3 py-2 text-sm" value={form.productName} onChange={e=>setForm(f=>({...f,productName:e.target.value}))} />
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="block text-sm text-gray-600 mb-1">BOM</label>
