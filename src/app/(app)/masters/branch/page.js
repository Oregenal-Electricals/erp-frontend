...
export default function BranchListPage() {
  ...
  const toggleStatus = async (id, name) => {
    ...
    try { await api.patch(`/masters/branches/${id}/toggle-status`); fetchBranches(); }
  ...
  const columns = [
    ...
    ...(loaded && can(Permission.BRANCH_EDIT) ? [{
      key: 'actions', label: 'Actions',
      render: (r) => (
        <div className="flex items-center gap-1">
          <button onClick={() => router.push(`/masters/branch/${r.id}/edit`)}
