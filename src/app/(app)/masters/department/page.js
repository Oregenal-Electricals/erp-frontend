...
export default function DepartmentListPage() {
  ...
  const fetchDepts = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await api.get('/masters/departments?includeInactive=true');
      setDepts(data);
  ...
  const toggleStatus = async (id, name) => {
    ...
    try { await api.patch(`/masters/departments/${id}/toggle-status`); fetchDepts(); }
  ...
  const columns = [
    ...
    ...(loaded && can(Permission.DEPARTMENT_EDIT) ? [{
      key: 'actions', label: 'Actions',
      render: (r) => (
        <div className="flex items-center gap-1">
          <button onClick={() => router.push(`/masters/department/${r.id}/edit`)}
            className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
            <Pencil size={14} />
          </button>
          <button onClick={() => toggleStatus(r.id, r.name)}
